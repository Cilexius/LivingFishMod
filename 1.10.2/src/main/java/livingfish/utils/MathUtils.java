...
public class MathUtils {
	...
	public static void pullEntitytoEntity(Entity puller, double modifier, Entity pulledEntity) {
		...
		double distanceY = (puller.posY + puller.getEyeHeight() + 1.0D) - (pulledEntity.posY + puller.getEyeHeight());
	...
	/**
	 * Returns a RayTraceResult of an entities view direction in a certain distance.
	 * @param entity The watcher - The entity that looks
	 * @param blockReachDistance The distance the rayTrace stops, if there was no hit 
	 * @param stopOnLiquid
	 * @param partialTicks Usually 1.0F
	 * @return A RayTrace or null
	 */
    public static RayTraceResult rayTrace(Entity entity, double blockReachDistance, boolean stopOnLiquid, float partialTicks) {
        Vec3d vec3d = MathUtils.getPositionEyes(entity, partialTicks);
