...
public class ItemFishBucket extends Item {
    ...
    public ActionResult<ItemStack> onItemRightClick(ItemStack stack, World world, EntityPlayer player, EnumHand hand) {
        ...
        if (rayTrace == null) {
            ...
            if (!world.isBlockModifiable(player, pos)) {
                ...
                boolean isTank = MiscUtils.isTank(world, pos);
                BlockPos pos1 = (flag1 || isTank) && rayTrace.sideHit == EnumFacing.UP ? pos : pos.offset(rayTrace.sideHit);

                if (!player.canPlayerEdit(pos1, rayTrace.sideHit, stack)) {
                    return new ActionResult(EnumActionResult.FAIL, stack);
                } else if (this.tryToPlaceContent(player, world, pos1, stack, isTank)) {
    ...
    public boolean tryToPlaceContent(@Nullable EntityPlayer player, World world, BlockPos pos, ItemStack stack, boolean isTank) {
        ...
        if (!world.isAirBlock(pos) && material.isSolid() && !isReplaceable) {
            return false;
        } else {
            if (world.provider.doesWaterVaporize()) {
                ...
                for (int k = 0; k < 8; ++k) {
                    world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, (double)x + Math.random(), (double)y + Math.random(), (double)z + Math.random(), 0.0D, 0.0D, 0.0D, new int[0]);
                }
            } else {
                if (!world.isRemote && (!material.isSolid() || isReplaceable) && !material.isLiquid()) {
